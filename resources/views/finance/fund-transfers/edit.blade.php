...
@section('title', 'Edit Transfer Dana')
@section('header', 'Edit Transfer Dana')
@section('content')
<div class="max-w-xl mx-auto animate-fade-in-up">
    <div class="bg-admin-surface border border-admin-border rounded-admin-lg p-6">
        <form method="POST" action="{{ route('finance.fund-transfers.update', $fundTransfer) }}" enctype="multipart/form-data" class="space-y-5">
            @csrf @method('PUT')
            <div>
                ...
                <select name="from_account_id" required
                    ...
                    @foreach($accounts as $id => $name)
                        <option value="{{ $id }}" {{ old('from_account_id', $fundTransfer->from_account_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="to_account_id" required
                        class="w-full mt-1 px-4 py-2.5 bg-admin-canvas rounded-admin-md border border-admin-border text-sm text-admin-ink focus:outline-none focus:ring-2 focus:ring-admin-indigo/25">
                    @foreach($accounts as $id => $name)
                        <option value="{{ $id }}" {{ old('to_account_id', $fundTransfer->to_account_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="amount" value="{{ old('amount', $fundTransfer->amount) }}" required min="1"
                           class="w-full mt-1 px-4 py-2.5 bg-admin-canvas rounded-admin-md border border-admin-border text-sm text-admin-ink focus:outline-none focus:ring-2 focus:ring-admin-indigo/25">
                ...
                <div>
                    ...
                    <input type="date" name="date" value="{{ old('date', $fundTransfer->date->format('Y-m-d')) }}" required
                           class="w-full mt-1 px-4 py-2.5 bg-admin-canvas rounded-admin-md border border-admin-border text-sm text-admin-ink focus:outline-none focus:ring-2 focus:ring-admin-indigo/25">
                </div>
            </div>
            <div>
                ...
                <textarea name="note" rows="3"
                          class="w-full mt-1 px-4 py-2.5 bg-admin-canvas rounded-admin-md border border-admin-border text-sm text-admin-ink focus:outline-none focus:ring-2 focus:ring-admin-indigo/25">{{ old('note', $fundTransfer->note) }}</textarea>
            </div>
            <div>
                ...
                @if($fundTransfer->attachment)
                    <div class="flex items-center gap-2 mt-1">
                        <a href="{{ Storage::url($fundTransfer->attachment) }}" target="_blank" class="text-xs text-admin-indigo hover:underline flex items-center gap-1">
