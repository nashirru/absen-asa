...
<div class="max-w-xl mx-auto animate-fade-in-up">
    <div class="bg-admin-surface border border-admin-border rounded-admin-lg p-6">
        <form method="POST" action="{{ route('finance.accounts.update', $account) }}" class="space-y-5">
            @csrf @method('PUT')
            <div>
                <label class="text-xs font-semibold text-admin-slate">Nama Akun</label>
                <input type="text" name="name" value="{{ old('name', $account->name) }}" required maxlength="255"
            ...
            <div>
                <label class="text-xs font-semibold text-admin-slate">Saldo</label>
                <input type="number" step="0.01" name="balance" value="{{ old('balance', $account->balance) }}" required min="0"
                       class="w-full mt-1 px-4 py-2.5 bg-admin-canvas rounded-admin-md border border-admin-border text-sm text-admin-ink focus:outline-none focus:ring-2 focus:ring-admin-indigo/25">
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                          class="w-full mt-1 px-4 py-2.5 bg-admin-canvas rounded-admin-md border border-admin-border text-sm text-admin-ink focus:outline-none focus:ring-2 focus:ring-admin-indigo/25">{{ old('description', $account->description) }}</textarea>
            </div>
