...
<div class="space-y-4 animate-fade-in-up" x-data="dashboardApp()">
    @if(!$todayAbsensi || !$todayAbsensi->jam_masuk)
        <div class="flex items-start gap-3 p-4 rounded-xl bg-amber-50 border border-amber-100/70 text-amber-800 shadow-sm">
            <div class="w-8 h-8 rounded-lg bg-amber-100 flex items-center justify-center shrink-0">
                <i data-lucide="alert-triangle" class="w-4 h-4 text-amber-600"></i>
            </div>
            <div class="flex-1 space-y-0.5">
                <h4 class="text-xs font-bold uppercase tracking-wider text-amber-900">Perhatian</h4>
                <p class="text-xs text-amber-700 font-medium">Anda belum melakukan absensi masuk (Clock In) untuk hari ini. Silakan Clock In segera!</p>
            </div>
        </div>
    @endif
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-4 flex items-center justify-between shadow-sm gap-3">
        <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
                ...
                <p class="text-xs text-gray-500 truncate">{{ $siswa?->nis ?? '-' }} &bull; {{ $siswa?->kelas?->nama_kelas ?? 'Belum ada kelas' }}</p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
        <div class="grid grid-cols-2 gap-3">
            <div class="text-center">
                ...
                @if($todayAbsensi && $todayAbsensi->jam_masuk)
                    @php
                        $jamMasukTime = \Carbon\Carbon::parse($todayAbsensi->jam_masuk);
                        $batasMasuk = \Carbon\Carbon::parse($jamMasuk);
                        $lateMinutes = max(0, $jamMasukTime->diffInMinutes($batasMasuk));
                    @endphp
                    <span class="inline-block mt-1.5 px-2 py-0.5 rounded-md text-[10px] font-semibold {{ $jamMasukTime->lte($batasMasuk) ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700' }}">
                        {{ $jamMasukTime->lte($batasMasuk) ? 'Tepat Waktu' : 'Terlambat '.$lateMinutes.'m' }}
                    </span>
                @endif
            </div>
            <div class="text-center">
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
        <div class="flex items-center gap-3">
            ...
            <div class="flex-1">
                <p class="text-sm font-semibold text-gray-900">Persentase Kehadiran</p>
                <p class="text-xs text-gray-500">Akumulasi absensi kelas</p>
            </div>
            <div class="text-right">
                <span class="text-xl font-bold text-blue-600">{{ $persentase }}%</span>
            </div>
        </div>
    </div>

    <!-- Progress & Nilai Pelatihan -->
    <div class="bg-white rounded-xl border border-gray-100 p-4 shadow-sm space-y-4">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
                <i data-lucide="award" class="w-4 h-4 text-blue-600"></i>
                <h3 class="font-semibold text-sm text-gray-900">Progress & Nilai Pelatihan</h3>
            </div>
            @if($siswa?->nilai_pelatihan)
                <span class="px-2.5 py-1 rounded-lg text-xs font-bold bg-green-50 text-green-700 border border-green-200">
                    Nilai: {{ $siswa?->nilai_pelatihan }}
                </span>
            @else
                <span class="px-2.5 py-1 rounded-lg text-xs font-medium bg-gray-50 text-gray-500 border border-gray-100">
                    Belum Dinilai
                </span>
            @endif
        </div>
        ...
        <div class="space-y-2">
            <div class="flex justify-between items-center text-xs">
                <span class="text-gray-500 font-medium">Progress Pelatihan</span>
                <span class="font-bold text-gray-900">{{ $siswa?->progress_pelatihan ?? 0 }}%</span>
            </div>
            <div class="w-full bg-gray-100 rounded-full h-2">
                <div class="bg-blue-600 h-2 rounded-full transition-all duration-500" style="width: {{ $siswa?->progress_pelatihan ?? 0 }}%"></div>
            </div>
        </div>
    </div>
        ...
        @forelse($jadwalHariIni as $j)
            <div class="flex items-center gap-3 p-3 rounded-lg bg-gray-50">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-gray-500 truncate mt-0.5">{{ $j->kelas->nama_kelas ?? '-' }} &bull; {{ $j->sensei->user->name ?? '-' }}</p>
                </div>
            </div>
        @empty
            <p class="text-center text-xs text-gray-400 py-4">Tidak ada jadwal kelas hari ini</p>
        @endforelse
    </div>
</div>
...
function dashboardApp() {
    return {
        ...
        updateTime() {
            const now = new Date();
            this.timeString = now.toLocaleTimeString('id-ID', { hour12: false });
        }
    }
}
</script>
